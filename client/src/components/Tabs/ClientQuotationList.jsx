...
export default function ClientQuotationList({ clientQuotation }) {

  // State to hold filtered data and search term
            ...
            <tbody>
              ...
              {filteredData && filteredData.map((data, index) => (
                <tr key={data._id} className="border-b cursor-pointer dark:border-gray-700 hover:bg-gray-50"
                  onClick={() => window.open(`/QuotationEdit/${data._id}`, "_blank")}>
                  <td className="px-4 py-3">{++index}</td>
                  <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{data.prefix} - {data.quoteNo}</th>
                  <td className="px-4 py-3">{data.subject}</td>

                  <td className="px-4 py-3">
                    {data.isFinal
                      ? <span className='text-green-600 font-semibold'>Invoiced</span>
                      : <span className='text-red-600 font-semibold'>Pending</span>
                    }
                  </td>
                  <td className="px-4 py-3">{new Date(data.quoteDate?.split("T")[0]).toLocaleDateString('en-GB')}</td>
                </tr>
              ))}
            </tbody>
          </table>
