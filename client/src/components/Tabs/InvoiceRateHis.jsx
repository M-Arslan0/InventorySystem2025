      ...
      <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              {invoiceRateHistory && invoiceRateHistory.map((data, index) => (
                <tr key={data._id} className="border-b dark:border-gray-700 cursor-pointer"
                  onClick={() => window.open(`/InvoiceEdit/${data.InvoiceData?._id}`, "blank")}>
                  <td className="px-4 py-3">{++index}</td>
                  <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{data.InvoiceData?.prefix} - {data.InvoiceData?.InvoiceNo}</th>
                  ...
                  <td className="px-4 py-3">{new Date(data.InvoiceData?.InvoiceDate?.split("T")[0]).toLocaleDateString('en-GB')}</td>
